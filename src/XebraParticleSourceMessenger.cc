	...
	m_pParticleSource(pParticleSource), m_bShootIon(false)
{
	...
	// particle direction
	m_pDirectionCmd = new G4UIcmdWith3Vector("/xebra/gun/direction", this);
	...
	// particle energy
	m_pEnergyCmd = new G4UIcmdWithADoubleAndUnit("/xebra/gun/energy", this);
	...
	m_pPositionCmd = new G4UIcmdWith3VectorAndUnit("/xebra/gun/position", this);
	...
	// ion 
	m_pIonCmd = new G4UIcommand("/xebra/gun/ion", this);
	...
	m_pTypeCmd = new G4UIcmdWithAString("/xebra/gun/type", this);
	...
	// source shape
	m_pShapeCmd = new G4UIcmdWithAString("/xebra/gun/shape", this);
	...
	// center coordinates
	m_pCenterCmd = new G4UIcmdWith3VectorAndUnit("/xebra/gun/center", this);
	...
	m_pHalfxCmd = new G4UIcmdWithADoubleAndUnit("/xebra/gun/halfx", this);
	...
	m_pHalfyCmd = new G4UIcmdWithADoubleAndUnit("/xebra/gun/halfy", this);
	...
	m_pHalfzCmd = new G4UIcmdWithADoubleAndUnit("/xebra/gun/halfz", this);
	...
	// radius of source  
	m_pRadiusCmd = new G4UIcmdWithADoubleAndUnit("/xebra/gun/radius", this);
	...
	// confine to volume(s)
	m_pConfineCmd = new G4UIcmdWithAString("/xebra/gun/confine", this);
	...
	m_pAngTypeCmd = new G4UIcmdWithAString("/xebra/gun/angtype", this);
	...
	// energy distribution
	m_pEnergyTypeCmd = new G4UIcmdWithAString("/xebra/gun/energytype", this);
	...
	m_pEnergyFileCmd = new G4UIcmdWithAString("/xebra/gun/energyspectrum", this);
	m_pEnergyFileCmd->SetGuidance("File containing energy spectrum");
	m_pEnergyFileCmd->SetParameterName("EnergySpectrum", false);

	// verbosity
	m_pVerbosityCmd = new G4UIcmdWithAnInteger("/xebra/gun/verbose", this);
...
void
XebraParticleSourceMessenger::SetNewValue(G4UIcommand * command, G4String newValues)
{
	...
	else if(command == m_pIonCmd)
	{
		...
		{
			G4cout << "Set /xebra/gun/particle to ion before using /xebra/gun/ion command";
			G4cout << G4endl;
		}
	}
